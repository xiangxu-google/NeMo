...
- Post attention norm, post MLP norm

- Post embedding scaling
...
class Gemma3Config(GPTConfig):
    ...
    flash_decode: bool = False  # not supported
    ...
    def configure_model(self, tokenizer, pre_process=None, post_process=None) -> "MCoreGPTModel":
        ...
        # Trick the megatron's RotaryEmbedding to init model.embedding successfully,
        # but model.embedding will be replaced below.
        ...
        # Custom embedding and rope for gemma3
...
class Gemma3Model(GPTModel):
    ...
    def __init__(
        ...
        super().__init__(config or Gemma3Config(), optim=optim, tokenizer=tokenizer, model_transform=model_transform)
...
class Gemma3LanguageModelEmbedding(LanguageModelEmbedding):
    ...
    def forward(self, input_ids: Tensor, position_ids: Tensor, tokentype_ids: int = None) -> Tensor:
        ...
        embeddings = embeddings * torch.tensor(
            self.config.hidden_size**0.5, dtype=embeddings.dtype, device=embeddings.device
        )
        return embeddings
...
# TODO: this hangs when using multiple GPUs
